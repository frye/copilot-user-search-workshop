...
try {
  ...
  if (options['--exercise']) {
    ...
    if (id === '09-spec-kit') {
      const base = 'workshop/artifacts/spec-kit-reference';
      const required = [
        'README.md', 'constitution.md', 'checkpoint-template.md',
        '001-team-filter/spec.md', '001-team-filter/plan.md', '001-team-filter/research.md',
        '001-team-filter/data-model.md', '001-team-filter/contracts/user-search.md',
        '001-team-filter/quickstart.md', '001-team-filter/tasks.md',
        '001-team-filter/checklists/requirements.md', '001-team-filter/checklists/api.md',
        '001-team-filter/analysis.md', '001-team-filter/provenance.json',
      ];
      for (const path of required) {
        if (!fileState(root, `${base}/${path}`)?.length) throw new Error(`Missing Spec Kit reference artifact: ${path}`);
      }
      const provenance = json(resolve(root, `${base}/001-team-filter/provenance.json`));
      if (!/^\d+\.\d+\.\d+$/.test(provenance.observedVersion) || provenance.learnerEvidence !== false ||
          provenance.integration !== 'copilot' || provenance.featureDirectory !== 'specs/001-team-filter') {
        throw new Error('Invalid Spec Kit reference provenance.');
      }
      const spec = fileState(root, `${base}/001-team-filter/spec.md`).toString();
      if (!spec.includes('case-insensitive exact') || !spec.includes('blank `team`') ||
          !spec.includes('unknown non-blank team') || !spec.includes('team must occur at most once')) {
        throw new Error('Incomplete team-filter specification.');
      }
      const tasks = fileState(root, `${base}/001-team-filter/tasks.md`).toString();
      const taskLines = tasks.split(/\r?\n/).filter(line => line.startsWith('- [ ] T'));
      if (taskLines.length < 8 || taskLines.some(line => !/^- \[ \] T\d{3}(?: \[P\])?(?: \[US\d+\])? .+\S$/.test(line))) {
        throw new Error('Spec Kit tasks must use complete dependency-ready checklist entries.');
      }
      const apiChecklist = fileState(root, `${base}/001-team-filter/checklists/api.md`).toString();
      if (!/CHK001/.test(apiChecklist) || !/reviewer-owned quality gate/i.test(apiChecklist)) throw new Error('Missing reviewer-owned API requirements checklist.');
    }
