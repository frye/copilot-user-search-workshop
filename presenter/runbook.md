...
- `starter-v1.1` is immutable consumer base; current main includes its exact pin and examples release metadata. `starter-v1` is preserved but superseded, not moved.
...
- `starter-v1.1` itself contains the previous starter lock by design; current main's consumer helper carries the reviewed current locks into the new consumer. Clone main for entry, never the bootstrap tag directly.
